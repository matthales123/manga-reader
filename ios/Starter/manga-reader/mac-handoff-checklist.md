...
## ATS Setup For Local HTTP (Development)
If your backend URL is `http://` on LAN, add an ATS exception in Xcode:
1. Target -> `Info`
2. Add `App Transport Security Settings` (Dictionary)
3. Add `Exception Domains` (Dictionary)
4. Add your host (example: `192.168.0.239`) as a Dictionary inside Exception Domains
5. Under that host dictionary, set:
   - `NSIncludesSubdomains` = `NO`
   - `NSTemporaryExceptionAllowsInsecureHTTPLoads` = `YES`
   - `NSTemporaryExceptionMinimumTLSVersion` = `TLSv1.0`

Use this only for development LAN testing.
