...
struct OfflineSeriesStorageSummary: Identifiable {
    let id: String
    let title: String
...
final class OfflineLibraryStore: ObservableObject {
    ...
    func downloadedSeriesStorageSummaries() -> [OfflineSeriesStorageSummary] {
        let grouped = Dictionary(grouping: downloadedByID.values) { $0.seriesID }

        return grouped.map { seriesID, volumes in
            let totalBytes = volumes.reduce(Int64(0)) { partial, volume in
                partial + volumeBytes(volume)
            }

            return OfflineSeriesStorageSummary(
                id: seriesID,
                title: volumes.first?.seriesTitle ?? "Offline Series",
                chapterCount: volumes.count,
    ...
    func totalOfflineBytes() -> Int64 {
        downloadedByID.values.reduce(Int64(0)) { partial, volume in
            partial + volumeBytes(volume)
        }
    }
    ...
    func removeDownloads(forSeriesID seriesID: String) throws {
        let ids = downloadedByID.values
            .filter { $0.seriesID == seriesID }
            .map(\.id)

        for id in ids {
            let finalDir = directoryURL(for: id)
            if fileManager.fileExists(atPath: finalDir.path) {
                try fileManager.removeItem(at: finalDir)
            }
        }
    ...
    private func volumeBytes(_ volume: OfflineVolume) -> Int64 {
        volume.pageURLs.reduce(Int64(0)) { partial, url in
            let byteValue = (try? fileManager.attributesOfItem(atPath: url.path)[.size] as? NSNumber)?.int64Value ?? 0
            return partial + byteValue
        }
    }
